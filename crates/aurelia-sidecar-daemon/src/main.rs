...
async fn main() -> anyhow::Result<()> {
    ...
    let jellyfin = if let (Some(url), Some(api_key)) = (&config.jellyfin_url, &config.jellyfin_api_key) {
...
async fn try_read_sidecar_lyrics(audio_path: &PathBuf) -> anyhow::Result<ParsedLyrics> {
...
async fn find_sidecar_file(audio_path: &PathBuf) -> Option<PathBuf> {
