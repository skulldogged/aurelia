...
struct FeaturedHeroView: View {
    ...
    var body: some View {
        ...
        let cardHeight: CGFloat = isWide ? 280 : 240
            ...
            if let baseAlbum {
                ZStack {
                    if let nextAlbum, nextAlbum.id != baseAlbum.id {
                        featuredCard(for: nextAlbum, imageSize: imageSize, cardHeight: cardHeight, isWide: isWide)
                            .opacity(progress)
                            .allowsHitTesting(false)
                    }

                    featuredCard(for: baseAlbum, imageSize: imageSize, cardHeight: cardHeight, isWide: isWide)
                        .opacity(1 - progress)
                        .allowsHitTesting(false)
                }
    ...
    private func featuredCard(for album: FeaturedAlbum, imageSize: CGFloat, cardHeight: CGFloat, isWide: Bool) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: AureliaRadius.l, style: .continuous)
                .stroke(Color.white.opacity(colorScheme == .dark ? 0.18 : 0.26), lineWidth: 1)
        )
    }
