...
public class ApiService {
  ...
  private string? GetServerUrl() {
    return _sessionService.Credentials?.ServerUrl;
  }

  private string? GetToken() {
    return _sessionService.Credentials?.AccessToken;
  }

  private string? GetUserId() {
    return _sessionService.Credentials?.UserId;
  }
  ...
  public Task<HomeData> GetHomeDataAsync() {
    ...
    try {
      Song[] songs = LoadCachedSongsInternal();

      MobileHomeData homeData = AureliaCore.AureliaCore.DeriveMobileHomeData(
          songs,
          12,
          20,
          16,
  ...
  public Task<List<AureliaCore.Song>> GetSongsAsync(string? artistId = null, string? albumId = null, int limit = 100, int startIndex = 0) {
    List<Song> songs = LoadCachedSongsInternal().ToList();
  ...
  public async Task<AureliaCore.Playlist?> GetPlaylistAsync(string playlistId) {
    ...
    try {
      Song[]? songs = await AureliaCore.AureliaCore.GetPlaylistItems(serverUrl, token, playlistId);
  ...
  public Task ReportPlaybackStartAsync(string itemId) {
    return Task.CompletedTask;
  }

  public Task ReportPlaybackProgressAsync(string itemId, long positionTicks) {
    return Task.CompletedTask;
  }

  public Task ReportPlaybackStoppedAsync(string itemId, long positionTicks) {
    return Task.CompletedTask;
  }

  public async Task<aurelia_lyrics.ParsedLyrics?> GetParsedLyricsAsync(
        string songId, string artist, string title) {
    string serverUrl = GetServerUrl() ?? "";
    string token = GetToken() ?? "";
    string? lyricsServerUrl = _sessionService.GetLyricsServerUrl();
    try {
      ParsedLyrics lyrics = await AureliaCore.AureliaCore.GetParsedLyrics(
          serverUrl, token, songId, artist, title, null, lyricsServerUrl);
      bool hasContent = (lyrics.plain?.Length > 0) || (lyrics.synced?.Length > 0);
      return hasContent ? lyrics : null;
    } catch (Exception ex) {
      Logger.Error($"[GetParsedLyricsAsync] {ex.Message}");
      return null;
    }
  }
  ...
  public async Task<bool> ToggleFavoriteAsync(string itemId, bool isFavorite) {
    ...
    Logger.Info($"[ToggleFavoriteAsync] serverUrl={serverUrl}, token={token?[..Math.Min(10, token?.Length ?? 0)]}..., userId={userId}");
    ...
    try {
      bool result = await AureliaCore.AureliaCore.ToggleFavorite(serverUrl, token, userId, itemId, isFavorite);
  ...
  public async Task<List<string>> GetFavoriteIdsAsync() {
    ...
    try {
      string[] result = await AureliaCore.AureliaCore.GetFavoriteIds(serverUrl, token, userId);
  ...
  public async Task SyncLibraryAsync() {
    ...
    try {
      SyncState stateBefore = AureliaCore.AureliaCore.GetSyncState(_appDataDir);
    ...
    Logger.Info($"[SyncLibraryAsync] serverUrl={serverUrl}, token={token?[..Math.Min(10, token?.Length ?? 0)]}..., userId={userId}");
    ...
    try {
      ...
      SyncReport report = await AureliaCore.AureliaCore.SyncLibrarySmart(serverUrl, token, userId, _appDataDir);
      Logger.Info($"[SyncLibraryAsync] Sync complete: {report}");

      // Log sync state after sync
      try {
        SyncState stateAfter = AureliaCore.AureliaCore.GetSyncState(_appDataDir);
      ...
      Song[] songs = AureliaCore.AureliaCore.LoadCachedSongs(_appDataDir) ?? [];
  ...
  public Task<string> GetStreamUrlAsync(string itemId, string? container = null) {
    ...
    try {
      ...
      string result = AureliaCore.AureliaCore.BuildStreamUrl(serverUrl, token, itemId, container);
      Logger.Info($"[GetStreamUrlAsync] Result: {(string.IsNullOrEmpty(result) ? "(empty)" : result[..Math.Min(100, result.Length)] + "...")}");
  ...
  public Task<string> GetFallbackStreamUrlAsync(string itemId, string? container = null) {
    ...
    try {
      string normalizedServer = serverUrl.TrimEnd('/');
      string escapedId = Uri.EscapeDataString(itemId);
      string escapedToken = Uri.EscapeDataString(token);
      string result = $"{normalizedServer}/Audio/{escapedId}/stream.aac?api_key={escapedToken}";
      Logger.Info($"[GetFallbackStreamUrlAsync] Building lossy fallback stream URL for itemId: {itemId}, container: {container ?? "(null)"}");
      Logger.Info($"[GetFallbackStreamUrlAsync] Result: {(string.IsNullOrEmpty(result) ? "(empty)" : result[..Math.Min(100, result.Length)] + "...")}");
      return Task.FromResult(result);
    } catch (Exception ex) {
  ...
  public Task<List<string>> GetLosslessFallbackStreamUrlsAsync(string itemId, string? container = null) {
    ...
    string escapedId = Uri.EscapeDataString(itemId);
    string escapedToken = Uri.EscapeDataString(token);
    string baseUrl = $"{normalizedServer}/Audio/{escapedId}/universal?api_key={escapedToken}";
